...
type Task struct {
	PID int	Name stringProgram stringCPU float64}
...
func (t *Tasky) GetWindowsTasks(callback func(Task)) ([]Task, error) {
	...
	r.FieldsPerRecord = -1	rows, err := r.ReadAll()
	if err != nil {
		return nil, err}

	// tasklist /fo csv columns:
	//0 imageName,1 pid,2 sessionName,3 sessionNumber,
	//4 memUsage,5 status,6 username,7 cpuTime,8 windowTitle	tasks := make([]Task,0, len(rows))
...
func (t *Tasky) GetUnixTasks(all bool, callback func(Task)) ([]Task, error) {
	...
	var (
		mu sync.Mutex	wg sync.WaitGroup	err error)
	...
	for _, col := range headers {
		...
		go func(cmdCol string) {
			...
			var buf bytes.Buffer	ps.Stdout = &buf	if e := ps.Run(); e != nil {
				setErr(e)
				return}
			...
			for _, line := range lines[1:] {
				...
				if st.task.IsFilled() && !st.emitted {
					st.emitted = true	taskCopy := st.task	tasks = append(tasks, taskCopy)
					mu.Unlock()
					callback(taskCopy)
					continue}
				mu.Unlock()
